...
| Transcription | Amazon Transcribe Streaming | Real-time WebSocket streaming from browser to Transcribe. Credentials from Cognito. No S3 or Lambda needed. |
...
- The frontend communicates with the backend exclusively through the AppSync GraphQL API (for data operations). Transcribe Streaming credentials come from the Cognito Identity Pool's unauthenticated role via `fetchAuthSession()` — no Lambda needed.
- DynamoDB authorization rules are defined per-model in the Amplify Data schema. Guest users can read questions and create submissions. Admin access requires authentication.
- The Cognito unauthenticated role has only `transcribe:StartStreamTranscription` permission — least-privilege by default.

### Speech Transcription Flow (Real-Time Streaming)
1. User taps "Record" in the browser.
2. The frontend calls `fetchAuthSession()` from `aws-amplify/auth` to get temporary AWS credentials from the Cognito Identity Pool. The unauthenticated role has `transcribe:StartStreamTranscription` permission.
...
- Lambda cannot hold a long-running WebSocket open (it's stateless and short-lived), so the WebSocket runs in the browser. Credentials come from Cognito — no Lambda needed at all.
...
3. On submission, all responses are saved as a JSON array inside a single `Submission` record via the AppSync API (`client.models.Submission.create()`).
...
│   ├── data/
│   │   └── resource.ts              # Data models (Question, Submission)
│   ├── backend.ts                    # Wires auth + data, grants Transcribe IAM to Cognito unauth role
│   ├── package.json
│   └── tsconfig.json
├── public/
├── src/
...
**Submission**
- `id` (auto-generated string)
- `respondentName` (string, optional) — name of the survey respondent
- `respondentEmail` (string, required) — email of the respondent
- `responses` (JSON, required) — array of `{ questionId, questionText, responseText }` objects
...
### Transcribe Streaming Access
- No Lambda function is needed. The `transcribe:StartStreamTranscription` IAM policy is attached directly to the Cognito Identity Pool's **unauthenticated role** in `backend.ts`.
- The frontend calls `fetchAuthSession()` from `aws-amplify/auth` to get credentials, then uses `@aws-sdk/client-transcribe-streaming` to open a WebSocket to Transcribe.
...
```
(No `microphone-stream` — uses native Web Audio API to avoid Vite/Buffer compatibility issues.)

### Custom Hook (`src/hooks/useRealtimeTranscription.js`)
Manages three concerns:
1. **Microphone access** — captures audio via `getUserMedia` + Web Audio API `ScriptProcessorNode`, converts Float32 to 16-bit PCM `Uint8Array` at 16kHz
...
### IAM Setup (`amplify/backend.ts`)
The `transcribe:StartStreamTranscription` policy is attached to the Cognito unauthenticated role:
```typescript
...
```
No Lambda function or STS AssumeRole needed.
...
- **Do not call AWS services directly from the frontend** except for Amazon Transcribe Streaming, which requires a direct browser-to-AWS WebSocket for real-time performance. Credentials come from `fetchAuthSession()` (Cognito Identity Pool).
...
- No Lambda function is needed for transcription. Credentials come from Cognito directly.
...
- **Transcribe credentials** are provided by the Cognito Identity Pool's unauthenticated role — never hardcoded
...
- **WebSocket connection fails:** Verify the Cognito unauthenticated role has `transcribe:StartStreamTranscription` permission in `backend.ts`. Check the AWS region supports Transcribe Streaming.
