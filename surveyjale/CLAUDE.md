...
### Data Flow
1. `App.jsx` reads `?survey=<id>` from the URL on mount. If missing, the app renders an "Invalid survey link" screen immediately — no data is fetched.
2. Questions are fetched filtered by `surveyId`: `client.models.Question.list({ filter: { surveyId: { eq: surveyId } } })`, sorted by `order`.
3. User completes the info step (name + email), then fills in survey responses (typed or voice).
4. On submission, a `Submission` record is created with all responses and the `surveyId` tagged: `client.models.Submission.create({ ..., surveyId })`.
5. Each survey's questions and submissions are isolated by `surveyId` — running the seed script again creates a new independent survey.
...
│   ├── data/
│   │   └── resource.ts              # Data models (Survey, Question, Submission) + transcribeAudio mutation
│   ├── functions/
...
│       └── VoiceRecorder.css
├── seedquestions.js          # Creates a Survey + seeds its questions into DynamoDB; prints share link
...
- `name` (string, required) — human-readable label for the survey
- `questions` (hasMany → Question via `surveyId`)
- `submissions` (hasMany → Submission via `surveyId`)
...
- `surveyId` (ID, required) — foreign key to Survey
...
- `responses` (JSON, required) — array of `{ questionId, questionText, responseText }`
- `surveyId` (ID, required) — foreign key to Survey
- `survey` (belongsTo Survey)

**Custom Mutations**
- `transcribeAudio(audio: String): String` — returns a transcription result. Backed by the transcribe Lambda function.
...
# Create a new survey and seed its questions (prints a share link)
node seedquestions.js --name "Survey Name"

# For production links, pass the deployed app URL
...
### Seeding workflow
Each run of `seedquestions.js` creates a new `Survey` record and seeds all questions under it. The script prints the survey ID and a ready-to-share URL:

```
Survey created: "Survey Name" (id: abc-123)
Created: "Walk me through..."
...
Seeded 12 questions.
Share link: http://localhost:5173?survey=abc-123
```

Respondents visit that link — the app reads `?survey=abc-123` from the URL and fetches only that survey's questions. Submissions are tagged with the same `surveyId`. Old surveys and their submissions are unaffected.

If no `?survey=` param is present, the app shows "Invalid survey link" instead of an empty form.
...
3. Run `node seedquestions.js --name "Test"` — prints a link like `http://localhost:5173?survey=<id>`
4. Open that link to test the survey end-to-end
5. Edit backend files in `amplify/` — the sandbox auto-redeploys changes
6. Edit frontend files in `src/` — Vite hot-reloads changes
